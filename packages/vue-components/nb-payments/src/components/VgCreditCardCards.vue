...
export default defineComponent ({
  ...
  setup (props, { emit }) {
    ...
    const cardStyleBorderRadius = computed(() => {
      const valueIsNumber = isNumber(parseInt(cardRadius.value, 10))
      const borderRadius = (cardRadius.value && valueIsNumber) ? cardRadius.value : defaultCardStyle.radius
      return {
    ...
    const onMouseEnterCvv = () => {
      front.value.style.transform = 'perspective(1000px) rotateY(-180deg) rotateX(0deg) rotateZ(0deg)'
