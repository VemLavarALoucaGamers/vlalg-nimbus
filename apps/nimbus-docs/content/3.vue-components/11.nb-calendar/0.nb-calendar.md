...
| minWidth   | Number     | 200  | Minimum calendar width in pixels. Invalid values (non-number, NaN, or ≤ 0) use **200**. If greater than `maxWidth`, it is set to `maxWidth`. |
| maxWidth   | Number     | 280  | Maximum calendar width in pixels. Invalid values use **280**. If less than `minWidth`, it is set to `minWidth`. |
| widthFull   | Boolean     |  false  | When `true`, the calendar stretches to **100%** of the parent width. `minWidth` still applies; `maxWidth` is not capped by the `maxWidth` prop (uses **100%** of the parent). When `false`, the calendar uses `width: 100%` inside its box, clamped between `minWidth` and `maxWidth` (responsive to the parent/container size). |
...
### Calendar with Custom Width Bounds

The calendar has no fixed `width` prop. Size is controlled by `minWidth` / `maxWidth` and the parent container (defaults **200**–**280** px).

```html
<template>
  <div style="width: 320px;">
    <NbCalendar
      nb-id="calendar-bounds"
      :min-width="200"
      :max-width="320"
      @changed="onDateChanged"
    />
...
Use inside a container with a defined width. With `width-full`, the calendar fills the parent; `minWidth` still applies.
