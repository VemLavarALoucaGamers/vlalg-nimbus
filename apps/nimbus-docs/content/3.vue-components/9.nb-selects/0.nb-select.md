...
| emptyOptionText   | String     | 'Select an option'  | Text for the empty option. In the closed field, with **showLabel**, this placeholder is shown only while the select is focused/open. Without a label, it is always shown when nothing is selected. |
...
| showLabel   | Boolean     | false  | Defines if the label should be displayed. When `true`, shows a floating label that moves to the top when the select is focused/open or has a selected value. |
