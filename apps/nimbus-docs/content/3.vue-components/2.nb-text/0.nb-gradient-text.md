...
| alignment   | String     | 'left'       | Defines the text alignment. Accepts **center**, **left** and **right**. For this property to work, the component needs to have sufficient height and be in a container with defined size, or the text to be large within a container of defined size. |
| paddingBottom    | String     |  '5'      | Defines text padding-bottom. Depending on the font size and if the last paragraph has a letter with a long leg, the leg of the letter will have a hidden part. This property allows to fix this. The minimum value is 5 |
