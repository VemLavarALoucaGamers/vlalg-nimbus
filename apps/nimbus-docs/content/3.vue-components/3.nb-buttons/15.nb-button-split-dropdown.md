...
| title     | String     | ''      | Tooltip text shown when hovering over the main button. |
...
| ------- | :--------- | :---------------------------- |
| clicked | Object, MouseEvent \| KeyboardEvent | Fired when any button is clicked. First argument: object with `type` ('main' or 'menu-item'); if 'menu-item', also `item` and `index`. Second argument: native DOM event. |
