...
| disabled     | Boolean     | false    | When `true`, disables all interactions: blocks the magnifier glass, prevents opening the preview modal, and closes the modal if it is already open. Sets `aria-disabled` and applies the disabled visual state |
...
| ------- | :--------- | :---------------------------- |
| clicked | MouseEvent \| KeyboardEvent | Fired when the image is clicked. Receives the native DOM event as first argument. |
