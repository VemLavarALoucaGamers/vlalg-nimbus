...
| hasThumbnailCustomContent     | Boolean     | false    | Enables a custom overlay content slot for each thumbnail. When true, provide dynamic slots using `thumbnail-image-{index}` |
...
| thumbnail-image-{index} | Custom content rendered over a thumbnail at a specific index. Use dynamic slot names like `#['thumbnail-image-0']` or `#[\`thumbnail-image-\${index}\`]`. Receives `images`, `image`, `index`, and `slotName` |
...
| ------- | :--------- | :---------------------------- |
| clicked | Object, MouseEvent \| KeyboardEvent | Fired when a thumbnail is clicked. First argument: `{ index: number, image: object }`. Second argument: native DOM event. |
...
### Custom Thumbnail Content (Remove "X" Button)

```vue
<template>
  <NbImageGallery
    nb-id="gallery-thumbnail-custom-content"
    :images="images"
  ...
  >
    ...
    >
      <div class="thumbnail-remove-wrap">
        <div
          class="thumbnail-remove-button"
          @click.stop="removeImage(slotProps.index)"
        >
          X
        </div>
      </div>
...
/* optional: prevent clipping when the badge is outside the thumbnail bounds */
:deep(.component__thumbnail-inner) {
  overflow: visible;
}
</style>
```
