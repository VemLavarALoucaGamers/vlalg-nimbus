...
| inputMask   | String, Array     | null  | Input mask using [`vue-the-mask`](https://www.npmjs.com/package/vue-the-mask) (same library as `NbCreditCard` in `@vlalg-nimbus/nb-payments`). **Only when `input-type="text"`** (ignored for `number`, `email`, `password`). String (e.g. CPF `###.###.###-##`) or **array** of patterns for variable length (e.g. phone `(##) ####-####` vs `(##) #####-####`, or BRL with growing groups like `R$ #,##` … `R$ ###.###.###,##`). Emits the **masked string** in `changed` / `current-value`. Template: `input-mask`. |
