...
| min   | String     | ''  | Minimum **numeric value** (native HTML `min`). Only when `inputType="number"`. Empty string does not set the attribute. Example: `min="0"`. Does **not** limit character count. |
| max   | String     | ''  | Maximum **numeric value** (native HTML `max`). Only when `inputType="number"`. Empty string does not set the attribute. Example: `max="10"` means values above **10** are out of range. Does **not** limit character count. |
| step   | String, Number     | ''  | Valid value increment (native HTML `step`). Only when `inputType="number"`. Number (e.g. `1`, `0.5`) or `"any"` (any decimal). Empty = attribute omitted (browser default is usually `1`). |
| maxlength   | Number     | null  | Maximum **character count** (native HTML `maxlength`). Only when `inputType` is **text**, **email**, or **password**. `null` = no limit. Not applied for `number` (browsers ignore `maxlength` on number inputs — use `max` for the numeric ceiling). |
| minlength   | Number     | null  | Minimum **character count** (native HTML `minlength`). Only when `inputType` is **text**, **email**, or **password**. `null` = no minimum. Not applied for `number`. |
...
### Number value limits (`min`, `max`, `step`)

Only when **`inputType="number"`**. These map to the native HTML attributes and constrain the **numeric value**, not the number of characters typed.
...
- `max="10"` → values greater than **10** are out of range for the browser.
- `step="0.5"` → valid steps like 0, 0.5, 1, 1.5…
- `step="any"` → any decimal is allowed.

### Character limits (`minlength`, `maxlength`)

Only when **`inputType`** is **`text`**, **`email`**, or **`password`**. These map to native HTML `minlength` / `maxlength`.
...
Not applied for `inputType="number"` — use `min` / `max` for the numeric range instead.
