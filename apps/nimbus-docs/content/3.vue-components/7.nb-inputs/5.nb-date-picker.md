...
| calendarMaxWidth   | Number     |  280    | Maximum popup width in pixels. Passed to `NbCalendar` `maxWidth`. Invalid values use **280**; if less than `calendarMinWidth`, uses `calendarMinWidth`. The popup width follows the **input width** while open (updated on input resize), clamped between these values. |
