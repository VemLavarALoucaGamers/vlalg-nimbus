...
| calendarMaxWidth   | Number     |  280    | Maximum popup width in pixels. Passed to `NbCalendar` `maxWidth`. Invalid values use **280**; if less than `calendarMinWidth`, uses `calendarMinWidth`. The popup width is clamped between `calendarMinWidth` and this value, and may shrink on narrow viewports. |
