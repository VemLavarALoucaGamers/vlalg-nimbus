...
| calendarCloseOnSelect   | Boolean     |  false    | When `true` and `allowRange` is `false`, closes the calendar popup after a complete selection (`date`, `month`, `time`, `datetime-local`, or `week`). Ignored when `allowRange` is `true`. |
| calendarCloseOnScroll   | Boolean     |  true    | When `true`, closes the popup on page/element scroll (capture). When `false`, keeps it open and recalculates position. |
| calendarCloseOnResize   | Boolean     |  true    | When `true`, closes the popup on window resize. When `false`, keeps it open and recalculates position. |
...
| calendarMinWidth   | Number     |  200    | Minimum popup width in pixels. Passed to `NbCalendar` `minWidth`. Invalid values use **200**; if greater than `calendarMaxWidth`, uses `calendarMaxWidth`. |
| calendarMaxWidth   | Number     |  280    | Maximum popup width in pixels. Passed to `NbCalendar` `maxWidth`. Invalid values use **280**; if less than `calendarMinWidth`, uses `calendarMinWidth`. The popup uses this value as its width (unless `calendarWidthFull` is `true`). |
...
| calendarWidthFull   | Boolean     |  false    | Passed to `NbCalendar` `widthFull`. When `true`, the popup stretches to the parent width; `calendarMinWidth` still applies. See [NbCalendar](/vue-components/nb-calendar/nb-calendar) for width behavior. |
