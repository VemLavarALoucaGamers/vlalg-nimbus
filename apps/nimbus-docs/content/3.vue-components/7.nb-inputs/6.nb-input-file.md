...
| showConstraintsAccept | Boolean | `true` | When `true` and `file-extension` / `file-extensions` and/or `allowed-extensions` is set, appends the **accepted** line (see `locale.acceptedLine`) |
...
| `removeAt(index)` | `void` | Removes the file at `index` in the component’s internal list (same order as in `changed` / `current-value`), syncs the native input when needed, then emits `changed` + `current-value`. |

Each `File` emitted in `changed` / `current-value` has an own numeric **`index`** property: its **current** position in that list (`0`…`n-1`), updated whenever the list changes inside the component (new selection, remove one, clear). If you reorder a **separate** UI only, rely on the last payload or call `removeAt` with the up-to-date index.
...
<template>
  ...
  />
  <button type="button" @click="removeAllFiles()">Clear from parent</button>

  <ul>
    <li v-for="(file, index) in files" :key="index">
      {{ file.name }} <button @click="removeFile(file.index)">remove</button>
    </li>
  </ul>
</template>
...
| ---- | ------- | ----------- |
| changed | `File[]` | Current list after merge with a new selection; each item may include **`file.index`** (see [Methods (template ref)](#methods-template-ref)) |
...
| acceptedLine | `{accept}`, `{extensions}` | **One string**, two placeholders: `{accept}` = MIME / `accept` (from `file-extension` / `file-extensions`), `{extensions}` = comma-separated extensions (from `allowed-extensions`). **Default** `Accept: {accept} · Extensions: {extensions}`: if only MIME or only extensions is set, the built-in default collapses to a short single-part line (`Accept: …` or `Extensions: …`). If you override `acceptedLine`, both variables are always passed (empty string when missing) — use a template that omits what you do not want to show. |
...
<NbInputFile
  :locale="{
    ...
    acceptedLine: 'MIME {accept} · ext {extensions}'
  }"
/>
```
