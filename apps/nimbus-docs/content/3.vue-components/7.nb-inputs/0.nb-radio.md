...
| currentOption   | String     | ''  | Defines the selected value. The value must exist in **options** under the **valueKey** property.  |
...
| clicked | MouseEvent \| KeyboardEvent | Fired when a radio button is clicked. Receives the native DOM event as first argument. |
