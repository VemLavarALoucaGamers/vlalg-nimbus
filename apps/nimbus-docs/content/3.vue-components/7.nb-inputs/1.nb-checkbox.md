...
| options   | Array     | []  | Defines the list of options. Each item is an object; by default **value** and **text** are used (customize via **valueKey** and **textKey**). |
| valueKey   | String     | 'value'  | Key name for the value property in option objects. |
| textKey   | String     | 'text'  | Key name for the text/label property in option objects. |
| currentOption   | Array     | []  | Defines an array of selected values. Each value must exist in **options** under the **valueKey** property.  |
| theme   | String     | 'light'  | Defines the theme. Accepts **dark** and **light**. When theme is set, uses theme-specific color props. |
| boxRadius    | Number     |  0    | Defines border-radius for **type** `box`. |
| color    | String     |  '#767676'    | Defines the checkbox color (icon/check). Accepts **Color name** and **Hex** |
...
| clicked | MouseEvent \| KeyboardEvent | Fired when a checkbox is clicked. Receives the native DOM event as first argument. |
